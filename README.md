Надо доделать:
- Сейчас сделал заплатку на фиксированный путь бэкапов ms sql на сервере. Такое ощущение, что у автора скрипта агент дженкисна, дженкинс, сервер 1с и SQL был на одной машине.
- Переписать скрипт очистки пред базы и кэшей. Сейчас он использует com для администрирования и com падает на вызове **ConnectAgent**. (copy_etalon\drop_db.ps1 25 строка). С сам Com я разобрался. Сейчас этап очистики просто закоменнтирован
- 
